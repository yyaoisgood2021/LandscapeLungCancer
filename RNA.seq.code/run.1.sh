...
mv $3/fastqc_trim_results/*_val_1.fq.gz $3/fastqc_trim_results/1.fq.gz
mv $3/fastqc_trim_results/*_val_2.fq.gz $3/fastqc_trim_results/2.fq.gz

#####mapping#####
mkdir $3/mapping
STAR --runThreadN 10 --runMode alignReads --readFilesIn $3/fastqc_trim_results/1.fq.gz $3/fastqc_trim_results/2.fq.gz  --readFilesCommand zcat --genomeDir /home/lina/star.reference/star.index.hg19/  --outFileNamePrefix $3/mapping/STARoutput --outSAMtype BAM SortedByCoordinate --outReadsUnmapped unmapped_ 
...
featureCounts -p -t exon -g gene_name -T 8 -B -Q 30 -O -a ~/star.reference/hg19.gtf.gff/gencode.v19.annotation.gtf -o $3/count/$4.outputfile $3/mapping/STARoutputAligned.sortedByCoord.out.bam

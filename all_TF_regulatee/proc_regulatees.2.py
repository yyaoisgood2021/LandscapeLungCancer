...
for TF_up in increased_TF_list:
    result_df_this_tf_of_interest = result_df[(result_df['start']==TF_up)].sort_values('diff', ascending=False, ).copy() 
    # print(result_df_this_tf_of_interest)
    result_df_this_tf_of_interest.to_csv(os.path.join(save_folder,
                                                      'sorted_edges.up_TF.{}.txt'.format(TF_up)), sep='\t', index=None, )
